fetch("http://localhost:3000/api/products") //appel au Back
  .then((res) =>  res.json()) //callback " c'est la réponse reçu aprés la requéte de fetch"
...
function addProducts(data) {
 ...
 // changer data en canaps 
  data.forEach((canape) => {
    ...
    // la mise des informations sous format tableau 
    ...
    appendElementsToArticle(article, [image, h3, p]); // simplification du code
    appendArticleTosofa(sofa, article); //simplification du code 
  });
}
// c'est la fonction qui recupere les donnees du tableaux depuis l'array pour les injecter dans la section items
function appendElementsToArticle(article, array) {
  array.forEach((item) => {
    article.appendChild(item);// injection dans la page
  });
}
